...
public class EntityFallingBlock extends Entity {
    ...
    public EntityFallingBlock(World worldIn, double x, double y, double z,BlockPos ori, IBlockState fallingBlockState) {
        super(worldIn);
        this.fallTile = fallingBlockState;
        this.preventEntitySpawning = true;
        this.setSize(0.98F, 0.98F);
...
//                        }
                    ...
                    {
                        if (!flag1 && BlockFallingEx.canFallThrough(this.world.getBlockState(new BlockPos(this.posX, this.posY - 0.009999999776482582D, this.posZ)))) {
                            this.onGround = false;
                            return;
                        }
                    }
                    ...
                    if (iblockstate.getBlock() != Blocks.PISTON_EXTENSION) {
                        ...
                        if (!this.dontSetBlock) {
                            if (this.world.mayPlace(block, blockpos1, true, EnumFacing.UP, (Entity) null) && (flag1 || !BlockFallingEx.canFallThrough(this.world.getBlockState(blockpos1.down()))) && this.world.setBlockState(blockpos1, this.fallTile, 3)) {
                                if (block instanceof BlockFallingEx) {
                                    ((BlockFallingEx) block).onEndFalling(this.world, blockpos1, this.fallTile, iblockstate);
                                }
